...
build30USM <- function(ind, indlot) {
  ...
  #=======================
  ListClimateModels <- c("0_0", "GFDL-CM3_45", "GISS-E2-R_45", "HadGEM2-ES_45", "MIROC5_45", "MPI-ESM-MR_45")

  for (iClimModel in 1:length(ListClimateModels)) {

    #M (added for Maize at head of rotation) Read Sowing Dates
    SowingDates <-
            read.csv(
              paste(
    ...
    nbplantes <- 1
    codesimul <- 0
    #M  fplt_1 <- file.path(DirPlant, "soybean_0_plt.xml", fsep = "")
    #M  ftec_1 <- file.path(DirTec, "Soja_tec.xml", fsep = "")
    ...
    fclim2 <-
    paste(DirClimate, dataGrid$CLocation[ind], "_v3test.1982", sep = "")
    #M fplt_1 <- file.path(DirPlant, "maizeP_grain_plt.xml", fsep = "")
    #M ftec_1 <- file.path(DirTec, "Mais_tec.xml", fsep = "")
    ...
    paste(DirClimate, dataGrid$CLocation[ind], "_v3test.1987", sep = "")
    #M fplt_1 <- file.path(DirPlant, "soybean_0_plt.xml", fsep = "")
    #Mftec_1 <- file.path(DirTec, "Soja_tec.xml", fsep = "")
    fplt_1 <- file.path(DirPlant, "maizeP_grain_plt.xml", fsep = "")
    ftec_1 <- file.path(DirTec, "Mais_tec.xml", fsep = "")
    USMYear1987 <-
    cbind.data.frame(
      ...
      for (iListProductionCase in 1:2) {
        ...
        for (iUSM in 1:nrow(USM30years)) {
          ...
          if (grepl("1981", USM30years[iUSM, ]$fclim1) == TRUE) {
            ...
            #Change maturity 
            #M (pas la peine car Mais) SticsRFiles::set_usm_txt(
            #M  filepath = paste(workspace_path, NomDir, "new_travail.usm", sep = "/"),
            #M  param = "fplt1",
            #M  value = paste("soybean_", ListMaturityGroup[iMatGroup], "_plt.xml", sep = ""))

            #M file.copy(
            #M     from = paste(
            #M      workspace_path,
            #M       "/InitCurrentUSMSoja/",
            #M       ListMaturityGroup[iMatGroup],
            #M       "ficplt1.txt" ,
            #M       sep = ""
            #M     ),
            #M     to = paste(workspace_path, NomDir, "ficplt1.txt", sep = "/") ,
            #M     overwrite = TRUE
            #M   )
            ...
            #M     from = paste(
            #M      workspace_path,
            #M       "/InitCurrentUSMSoja/fictec1.txt" ,
            #M       sep = ""
            #M     ),
            #M     to = paste(workspace_path, NomDir, "fictec1.txt", sep = "/") ,
            #M     overwrite = TRUE
            #M  )


            #M   #Change SowingDates
            #M   SticsRFiles::set_usm_txt(
            #M    filepath = paste(workspace_path, NomDir, "fictec1.txt", sep = "/"),
            #M     param = "iplt0",
            #M     value = SowingDatesCrop
            #M  )
            ...
            #Change crop for years with soybean 
            if (is.element(
              indyear,
              c(
                1982,
              ...
              #M     filepath = paste(workspace_path, NomDir, "new_travail.usm", sep = "/"),
              #M    param = "fplt1",
              #M   value = "maizeP_grain_plt.xml"
              #M )
              #M SticsRFiles::set_usm_txt(
              #M  filepath = paste(workspace_path,  NomDir, "new_travail.usm", sep = "/"),
              #M param = "ftec1",
              #Mvalue = "MaisDT_tec.xml"
              #M )
              #M   file.copy(
              #M     from = paste(
              #M       workspace_path,
              #M       "/InitCurrentUSMMais/ficplt1.txt" ,
              #M       sep = ""
              #M     ),
              #M     to = paste(workspace_path, NomDir, "ficplt1.txt", sep = "/") ,
              #M     overwrite = TRUE
              ...
              #M     from = paste(
              #M       workspace_path,
              #M       "/InitCurrentUSMMais/fictec1.txt" ,
              #M       sep = ""
              #M     ),
              #M     to = paste(workspace_path, NomDir, "fictec1.txt", sep = "/") ,
              #M     overwrite = TRUE
          ...
          if (linerefcurr == (linerefpred + 6)) {
            ...
            if (grepl("0_0", ListClimateModels[iClimModel]) == TRUE) {
              ...
              period <- "2"
              if (grepl("GFDL", ListClimateModels[iClimModel]) == TRUE) sce <- "GFDL-CM3_45"
              if (grepl("GISS", ListClimateModels[iClimModel]) == TRUE) sce <- "GISS-E2-R_45"
              if (grepl("Had", ListClimateModels[iClimModel]) == TRUE) sce <- "HadGEM2-ES_45"
              if (grepl("MIR", ListClimateModels[iClimModel]) == TRUE) sce <- "MIROC5_45"
              if (grepl("MPI", ListClimateModels[iClimModel]) == TRUE) sce <- "MPI-ESM-MR_45"
            }
